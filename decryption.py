...
if __name__ == '__main__':
    ...
    parser.add_argument(
            '--invert',
            help='decrypt a picture by inverting',
            action='store_true'
            )
    parser.add_argument(
            '--scramble',
            help='decrypt a picture by scrambling',
            action='store_true'
            )
    parser.add_argument(
            '--rotate',
            help='decrypt a picture by rotating',
            action='store_true'
            )
    parser.add_argument(
            '-i',
            '--input',
            help='specify a file to decrypt',
    ...
    for y in range(0, height, b_y):
        ...
        for x in range(0, width, b_x):
            ...
            if x == 0:
                ...
                img_result_row = cv2.hconcat([img_result_row, img])
        if y == 0:
            img_result = img_result_row
        else:
            img_result = cv2.vconcat([img_result, img_result_row])
